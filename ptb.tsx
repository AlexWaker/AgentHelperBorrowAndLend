// 只做 SUI 存款的 PTB 组件（async/await 版）
export function NaviDepositPTB() {
  ...
  const depositSui = async () => {
    ...
    try {
      ...
      if (!suiPool) throw new Error("Invalid SUI pool configuration");
      ...
      setMessage("");
      setDigest("");
      const result = await signAndExecuteTransactionAsync({ transaction: tx, chain: "sui:mainnet" });
      setDigest(result.digest);
    } catch (error) {
      console.error("Error in depositSui:", error);
  ...
  return (
    <Container>
      <Box>
        <Heading as="h2">Navi Protocol 存款任务</Heading>
        <Text>
          该任务会将 1 SUI 存入 Navi 协议的 SUI 池。
        </Text>
        <Flex gap="3" direction={"column"}>
          <Button onClick={depositSui} variant="solid">
            存入 1 SUI
          </Button>
          {digest && <Text>Transaction submitted: {digest}</Text>}
...
// const amountMist = naviService.toSuiMist(1); // 1 SUI
// const result = await naviService.supplySui({
//   from: account.address,
//   amountMist,
//   signer: ({ transaction, chain }) => signAndExecute({ transaction, chain }), // 或 mutateAsync
//   chain: 'sui:testnet', // 与实际网络一致
// });
